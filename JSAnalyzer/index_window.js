...
var traceFilter = function (content, options) {
	...
	try {
		...
		const rewriteWindow = function(read) {
			if (util.checkIfReservedWord(read) || read.source() == "this") {
				return;
			}
			if (read.source() == "arguments") {
				console.log(`INFO: says ${read.source()}`);
				return;
			}
		...
		var rewriteGlobals = function(globalArr) {
			if (options.useProxy) {
				globalArr.forEach(rewriteWindow);
			}
		}
			...
			*/
			...
			// or a local variable becoming an alias of a global variable
				...
				if (_functionId) {
					...
					// 	}
					// });
					readArrayT.forEach(rewriteWindow);
				...
				if (_functionId) {
					...
					/* Shaghayegh: comment these 3 lines if you want global
					 functions not to use window anymore */
					if (options.useProxy) {
						readArray.forEach(rewriteWindow);
					}
					rewriteClosure(antiLocal,_functionId);
					...
					// });
					if (options.useProxy) {
						globalReads.forEach(rewriteWindow);
					}
