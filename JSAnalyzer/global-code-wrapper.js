...
const util = require('./util.js'),
  scope = require('./scopeAnalyzer.js'),
...
function wrap(content, fala){
    ...
    try {
        ...
        content = fala({
            ...
            // if (isGlobalCode(node) && node.type != "Program"){
            //     //global code can't start at a iife already
            //     if (!globalStart) {
            //         if (node.type == "FunctionExpression" && node.parent.type == "CallExpression") return;
            //         globalStart = true && (globalCodeRange.s = node)
            //             && (globalCodeRange.e = node);
            //     }
            //     else {
            //         globalCodeRange.e = node;
            ...
            // } else {
            //     if (globalStart){
            //         update(globalCodeRange.s, '(function(){ ',globalCodeRange.s.source());
            //         update(globalCodeRange.e, globalCodeRange.e.source(), ' })() ');
            //         globalStart = false;
            //     }
