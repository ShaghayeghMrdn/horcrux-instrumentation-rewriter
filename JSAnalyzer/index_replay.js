...
var minFunctionTime = 0;// Minimum function time worth replaying is 2ms -> emperically decided
                ...
                if (node.type == "FunctionDeclaration" || node.type == "FunctionExpression") {
                    ...
                    if (options.myCg.indexOf(index)>=0){
                        ...
                        if (nodeTime == null || nodeTime <= minFunctionTime){
                            console.log("Marking node as null node");
                            node.nullTimeNode = true
                        } else {
                            staticInfo.rtiDebugInfo.totalNodes.push(options.cg[origCgInd]);
                            staticInfo.rtiDebugInfo.totalNodes.time += nodeTime;
                        }
                        node.time = nodeTime;
        ...
        ASTNodes.forEach(function(node) {
            ...
            */
            ...
            /* This function expression might be a part of the call expression as well however that will be taken care of inside the call expression check itself. */
                ...
                update(node.body, '\n', closures[0] ? closures[0] : '' ,' var __tracerRet; \n __tracerRet = ', _traceBegin, JSON.stringify(index)
                    ,replayObjects,') \n if (__tracerRet[0]) return __tracerRet[1]; \n',
                     node.body.source(),' \n');
                update(node.body, '{ \ntry {\n',options.tracer_name,'.cacheInit(', JSON.stringify(index),',arguments, new.target,null);\n',
                    node.body.source());
                ...
                update(node.body, node.body.source(),' \n} finally {',
                    ...
                    argumentRestoreStr ? argumentRestoreStr :null,
                     _traceEnd, JSON.stringify(index),',',JSON.stringify(null), ');\n }}');
