...
function limitProfileAtPLT(profile, plt, timestamps){
  var NEndTime = profile.startTime + plt*1000; //profile times are in micro seconds
  var NtimeStamps = timestamps.filter(e=>e<NEndTime);
  var Nlen = NtimeStamps.length;
  profile.samples = profile.samples.slice(0,Nlen);
  profile.timestamps = NtimeStamps;
  profile.endTime = NEndTime;
  profile.timeDeltas = profile.timeDeltas.slice(0,Nlen);

  //update hitcounts
  updateNodeHitCount(profile);
}

function updateNodeHitCount(profile){
  var nodeHitCounts = {};
  var ids = profile.nodes.map(e=>e.id);
  ids.forEach((id)=>{
    nodeHitCounts[id]=0;
  })
...
SDK.CPUProfileDataModel = class extends SDK.ProfileTreeModel {
  ...
  constructor(profile, plt) {
    ...
    if (plt){
      limitProfileAtPLT(profile, plt, this.timestamps);
      this.profileEndTime = profile.endTime/1000;
      this.timestamps = profile.timestamps;
    }
